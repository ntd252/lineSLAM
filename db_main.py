...
if __name__ == '__main__':
    ...
    max_r_distance = 220.0
    max_alpha_distance = 0.36
    ...
    # Read data.
    logfile = LegoLogfile()
    logfile.read("real_test/frame14/motor.txt")
    count = 0
    for i in range(len(logfile.motor_ticks)):
        frame_name = "real_test/frame14/frame" + str(i) + ".jpg"
        print("frame", i)
        frame = cv2.imread(frame_name)
        filtered_line_list = pose_estimate(frame)
        
        black = np.zeros((map_size,map_size),dtype=np.uint8)
        for pose in filtered_line_list:
            x1 = int(map_size/2) + pose[0]
            y1 = int(map_size/2) - pose[1]
            x2 = int(map_size/2) + pose[2]
            y2 = int(map_size/2) - pose[3]
            cv2.line(black, (x1, y1), (x2, y2), color=255, thickness=10)
        cv2.line(black, (0,int(map_size/2)), (map_size-1,int(map_size/2)), color=255, thickness=10)
        cv2.line(black, (int(map_size/2),0), (int(map_size/2),map_size-1), color=255, thickness=10)

        resized_pose = cv2.resize(black, (0,0), fx=0.1,fy=0.1)
        rotated_pose = cv2.rotate(resized_pose, cv2.ROTATE_90_COUNTERCLOCKWISE)
        pose_name = "real_test/frame14/pose" + str(i) + ".jpg"
        cv2.imwrite(pose_name, rotated_pose)

        # clear the stream in preparation for the next frame
        control = np.array(logfile.motor_ticks[i]) * [TICKS_TO_MM_L, TICKS_TO_MM_R]
        kf.predict(control)
        ...
        for wall in kf.lines:
            x1 = int(map_size/2) + wall[0]
            y1 = int(map_size/2) - wall[1]
            x2 = int(map_size/2) + wall[2]
            y2 = int(map_size/2) - wall[3]
            cv2.line(map_image, (x1, y1), (x2, y2), color=255, thickness=5)
        
        #draw position and heading angle of the car
        #heading vector
        headx = int(map_size/2) + int(kf.state[0] + 100*cos(kf.state[2]))
        heady = int(map_size/2) - int(kf.state[1] + 100*sin(kf.state[2]))
        statex_image = int(map_size/2) + int(kf.state[0])
        statey_image = int(map_size/2) - int(kf.state[1])
        cv2.circle(map_image, (statex_image, statey_image),radius=15,color=255,thickness=4)
        cv2.line(map_image, (headx, heady), (statex_image, statey_image), color=255, thickness=5)
        
        '''
        for predict in kf.predict_state:
            headx_pre = 3000 + int(predict[0] + 150*cos(predict[2]))
            heady_pre = 3000 - int(predict[1] + 150*sin(predict[2]))
            statex_image_pre = 3000+int(predict[0])
            statey_image_pre = 3000-int(predict[1])
            cv2.circle(map_image, (statex_image_pre, statey_image_pre),radius=15,color=255,thickness=4)
            cv2.line(map_image, (headx_pre, heady_pre), (statex_image_pre, statey_image_pre), color=255, thickness=5)
        '''

        map_name = "real_test/frame14/map" + str(i) + ".jpg"
        map_image = cv2.resize(map_image, (0,0), fx=0.3, fy=0.3)
        cv2.imwrite(map_name, map_image)
    ...
    cv2.imwrite("real_test/frame14/map.jpg", final_map)
    sys.exit()
